...
export class CertificateService {
    ...
    async getKeys(): Promise<[string, string]> {
        // if (!this.keysCreated) {
            this.keyPath = await promises.mkdtemp(join(tmpdir(), 'apple-store-card-certificate'));
            try {
                await promises.writeFile(join(this.keyPath, 'store-card.p12'),
                Buffer.from('APPLE_STORE_CARD_PK'), 'base64')

                await promises.writeFile(join(this.keyPath, 'wwdr.pem'),
                Buffer.from('APPLE_WWDR_PEM'), 'base64')

                await promises.writeFile(join(this.keyPath, 'tech.inspirare.wallet.pem'),
                Buffer.from('APPLE_STORE_CARD_PEM'), 'base64')
